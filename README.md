...
keytool.exe는 JDK를 설치한 bin 폴더에 있음 (C:\Program Files\Java\jdk1.8.0_121\bin)
...
> 마지막 assembleRelease까지 하면 이제 배포가 가능한.. android/app/build/outputs/apk/app-release.apk 라는 apk 파일이 생긴다.

성공~!

release APK 테스트
상용 배포 전에 테스트를 빡시게 해봐야 할 것이다.
기존 테스트용 apk는 지우고 아래 명령어를 실행하자.
...
Proguard는 APK의 크기를 살짝 줄여준다. (사용하지 않는 코드를 제거한다.)
Proguard는 가끔 library 별로 설정이 필요하다고 한다. ? 뭔지 잘 모르겠음.
